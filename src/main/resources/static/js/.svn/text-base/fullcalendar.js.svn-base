...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		html +=
			"<span class='fc-event-title' onclick='editEvent(\""+event.id+"\");'>" +
			htmlEscape(event.title || '') +
			"</span><span class='fc-event-close' onclick='deleteEvent(\""+event.id+"\");'><i class='fa' style='float:right;color:red' >x</i></span>" +
			"</div>";
