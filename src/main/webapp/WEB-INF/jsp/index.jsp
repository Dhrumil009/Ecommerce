                ...
                <ul class="customcontent-home">
                	<c:if test="${not empty catalogList }">
                		<c:forEach items="${catalogList}" var="item">
                			<li class="num-1"> <a href="getcategory?catalogID=${item.catalogID}"> <img src="${item.catalogImage}" title="${item.catalogName}" /> </a> </li>
                		</c:forEach>
                	</c:if>
                </ul>
              </div>
                		...
                		<c:forEach items="${productList }" var="item">
                			<li class="ajax_block_product span3   num-1 ">
	                    		<div class="featured_li"> 
			                    	<a class="product_image" href="productDetails?productID=${item.productID}" title="${item.productName}" rel="tooltip" data-placement="bottom" > 
...
<!-- 	                    		<span class="price-drop-feacherd">$128.00</span> -->
			                      <div class="box">
			                        ...
			                        <div class="clearfix"> </div>
			                        <a class="exclusive ajax_add_to_cart_button btn_add_cart" rel="ajax_id_product_1" href="addtoCart?productID=${item.productID}"> 
			                        	<span>Add to cart</span> </a> <a class="button" href="productDetails?productID=${item.productID}" rel="tooltip" data-placement="bottom">View</a> 
			                      </div>
	                    		</div>
	                  		</li>
                		</c:forEach>
